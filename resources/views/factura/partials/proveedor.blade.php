                        ...
                        <section class="text-gray-600 body-font">
                            <div class="container px-5 mx-auto">
                                <div class="flex flex-wrap -m-2">
                                    @foreach ($facturas as $reco)
                                    <div class="p-2 lg:w-1/3 md:w-1/2 w-full">
                                        <a href="{{ route('factura.update', ['factura' => $reco->id])}}">
                                            <div class="h-full flex items-center border-gray-200 border p-4 transform  hover:scale-105 transition duration-300 relative bg-clip-border rounded-3xl bg-white text-gray-700 shadow-md">
                                                <div class="flex-grow">
                                                    <h2 class="text-lg text-center  font-semibold mb-2">Factura {{$reco->id}}</h2>
                                                    <h2 class="text-gray-900 title-font font-medium">{{$reco->proveedor->marca}}</h2>
                                                    <p class="text-gray-500">{{$reco->proveedor->modelo}} {{$reco->proveedor->year}} {{$reco->proveedor->placas}} {{$reco->proveedor->serie}}</p>
                                                    <div class=" p-3 rounded-2xl mt-4">
                                                        <h3 class="text-md font-semibold mb-1">Información del factura</h3>
                                                        <p class="text-gray-600 mb-2">Fecha de factura: {{$reco->fecha_factura}}</p>
                                                        <p class="text-gray-600 mb-2">Tipo de factura: {{$reco->tipo_factura}}</p>
                                                        <p class="text-gray-600 mb-2">Costo: {{$reco->costo_factura}}</p>
                                                        <p class="text-gray-600 mb-2">Kilometraje: {{$reco->kilometraje}}</p>

                                                        <!-- Agrega más detalles si es necesario -->
                                                    </div>
                                                    <div class="bg-gray-100 p-3 rounded-2xl mt-4">
                                                        <h3 class="text-md font-semibold mb-1">Detalles del factura </h3>
                                                        <p class="text-gray-600 mb-2">{{$reco->detalle_factura}}</p>
